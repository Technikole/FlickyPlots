...
function initMap() {
    map = new google.maps.Map(document.getElementById("map"), {
        ...
        mapId: '93c2d724f0e150fab9fcbd41', // Unique Map ID GCP
        tilt: 45,
    });
    populateYearFilter();
    setupEventListeners();
    fetchAndProcessMovies(); // Fetch initial themed data
}
...
function setupEventListeners() {
    ...
    // (Your modal and theme toggle listeners go here)
}
...
async function fetchAndProcessMovies() {
    ...
    tableContainer.innerHTML = `<h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-300">Loading movies...</h2>`;

    // For vZombie - searchTerm from the input.
    const searchTerm = document.getElementById('search-input')?.value;

    try {
        const response = await fetch('/getQloo', {
            method: 'POST', // The smart function expects a POST to read the body
            headers: {
                'Content-Type': 'application/json',
                'X-Site-Version': 'zombie' // This header tells the backend which logic to run
            },
        ...
        tableContainer.innerHTML = `<p class="text-red-500">Could not load movie data.</p>`;
    }
}

async function fetchAiGuide(movie, elementId) {
    try {
        const response = await fetch('/getAiTourGuide', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-Site-Version': 'zombie' // Send header for the AI call too
            },
            body: JSON.stringify({
                title: movie.title,
        ...
        if (!response.ok) throw new Error('AI connection needs work! #FixMe');
        const data = await response.json();
        
        const aiContentDiv = document.getElementById(elementId);
        if (aiContentDiv) {
            const newHtml = `
                <h4 class="font-semibold text-sm mt-2 text-gray-800 dark:text-gray-200">Cultural Pairings</h4>
                ...
                <h4 class="font-semibold text-sm mt-3 text-gray-800 dark:text-gray-200">Travel Pitch</h4>
                <p class="text-sm text-gray-600 dark:text-gray-300">${data.pitch}</p>
            `;
...
// =================================================================================
// UI Rendering and Interaction (Your other functions go here)
// =================================================================================
...
function plotMoviesOnMap(movies) {
    ...
    const infoWindow = new google.maps.InfoWindow({ maxWidth: 550 });
        ...
        geocoder.geocode({ 'address': movie.filmingLocation }, (results, status) => {
            if (status === 'OK') {
                ...
                marker.addListener('click', () => {
                    const aiContentId = 'ai-content-' + movie.title.replace(/[^a-zA-Z0-9]/g, '');
                   const infoWindowContent = `
                        <div style="display: flex; align-items: flex-start; gap: 15px;">
                            ...
                            <div>
                                ...
                                <div id="${aiContentId}" style="padding-top: 8px; border-top: 1px solid #e5e7eb;">
                    ...
                    fetchAiGuide(movie, aiContentId);
 ...
 */
// New fetchAIGuide
async function fetchAiGuide(movie, elementId) { // It now accepts the unique elementId
    try {
        const response = await fetch('/getAiTourGuide', {
            method: 'POST',
        ...
        const data = await response.json(); // Data is an object { book, album, pitch }

        // Use the unique elementId to find the correct div
        const aiContentDiv = document.getElementById(elementId);
...
// =================================================================================
// Event Listeners and UI Logic (DOM-Safe - wrapper logic
// =================================================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (aboutButton && closeModalButton && modal) {
    ...
    let savedTheme = localStorage.getItem('color-theme') || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light');
...
//         zoom: 4,
//         center: { lat: 39.8283, lng: -98.5795 },
//         mapId: '93c2d724f0e150fab9fcbd41', // Make sure your real Map ID is here
//         tilt: 45,
//     });
//     populateYearFilter();
//     setupEventListeners();
//     fetchAndProcessMovies(); // Fetch initial data
// }
...
//     const genreCheckboxes = document.querySelectorAll('#genre-container input[type="checkbox"]:checked');
//     const genres = Array.from(genreCheckboxes).map(cb => cb.value);

//     // If a text search is active, clear other filters for better results
//     if (searchTerm) {
...
//     const filters = { searchTerm, country, year, genres };
...
// // Original fetchAiGuide function
...
// // =================================================================================
// // Theme Toggle & Modal Logic
// // =================================================================================
// // CHANGED TO: Wrapper logic
// // =================================================================================
